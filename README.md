...
- At least **90GB VRAM** (80GB+ recommended)
...
```bash
docker run --gpus all -p 10006:10006 --env-file .env pipeline:latest
```

- Start with docker run and env file and bound directory (Useful for active development)
```bash
docker run --gpus all -v ./pipeline_service:/workspace/pipeline_service -p 10006:10006 --env-file .env pipeline:latest
```
